...
sealed class StringValueImpl(StringType type, ValueStatus<string> value) : ValueImpl<StringValueImpl, StringType, string>(type, value), StringValue
{
    ...
    protected override string ValueToString(string value, IFormatProvider? fmtProvider, Indentation indent) => $"\"{Strings.Escape(value, fmtProvider)}\"";
}
sealed class StructureValue(StructureType type, ValueStatus<ImmutableOrderedMap<Ident, Value>> value)
    : ValueImpl<StructureValue, StructureType, ImmutableOrderedMap<Ident, Value>>(type, value)
{
