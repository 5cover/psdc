...
{
    Message(Range location, MessageCode code, FuncOrValue<string, string> content) => (Code, Location, Content) = (code, location, content);
    ...
    internal static Message ErrorAssertionFailed(Range location, Option<string> message) => new(location, MessageCode.AssertionFailed,
        message.Match(ms => Fmt($"compile-time assertion failed: {ms}"),
            () => "compile-time assertion failed"));
    internal static Message HintExpressionValueUnused(Range location) => new(location, MessageCode.ExpressionValueUnused,
        "value of expression unused");
    internal static Message ErrorIndexOutOfBounds(Range location, int index, int length) => new(location, MessageCode.IndexOutOfBounds,
        Fmt($"index out of bounds for array"),
        [Fmt($"indexed at {index}, length is {length}")]);
    internal static Message ErrorSwitchDefaultIsNotLast(Range location) => new(location, MessageCode.SwitchDefaultIsNotLast,
    ...
    internal static Message ErrorFeatureComingSoon(Range location, string feature) => new(location, MessageCode.FeatureNotAvailable,
        $"language feature '{feature}' not yet available");
    internal static Message ErrorConstantAssignment(Range location, Symbol.Constant constant) => new(location, MessageCode.ConstantAssignment,
        Fmt($"reassigning constant `{constant.Name}`"));
    internal static Message DebugEvaluateExpression(Range location, Value value) => new(location, MessageCode.EvaluateExpression,
        Fmt($"evaluated value: {value}"));
    internal static Message DebugEvaluateType(Range location, EvaluatedType type) => new(location, MessageCode.EvaluateType,
        Fmt($"evaluated type: {type:f}"));
    internal static Message ErrorUnsupportedInitializer(Range location, EvaluatedType initializerTargetType) => new(location,
        MessageCode.UnsupportedInitializer,
        Fmt($"unsupported initializer for type `{initializerTargetType}`"));

    internal static Message ErrorComptimeExpressionExpected(Range location) => new(location, MessageCode.ConstantExpressionExpected,
    ...
    internal static Message ErrorReturnInNonReturnable(Range location) => new(location, MessageCode.ReturnInNonReturnable,
        "return in something not a function, a procedure or a main program");
    internal static Message ErrorReturnExpectsValue(Range location, EvaluatedType targetType) => new(location, MessageCode.ReturnExpectsValue,
        Fmt($"return here requires a value of type '{targetType}'"));
    internal static Message ErrorRedefinedMainProgram(Range location) => new(location, MessageCode.RedefinedMainProgram,
    ...
    internal static Message ErrorCannotSwitchOnString(Range location) => new(location, MessageCode.CannotSwitchOnString,
    ...
    internal static Message ErrorUnsupportedDesignator(Range location, EvaluatedType targetType) => new(location, MessageCode.UnsupportedDesignator,
        Fmt($"unsupported designator in '{targetType}' initializer"));

    internal static Message ErrorStructureDuplicateComponent(Range location, Ident component) => new(location, MessageCode.StructureDuplicateComponent,
        Fmt($"duplicate component `{component}` in structure is ignored"));

    internal static Message ErrorNonIntegerIndex(Range location, EvaluatedType actualIndexType) => new(location, MessageCode.NonIntegerIndex,
        Fmt($"non integer ('{actualIndexType}') array index"));

    internal static Message ErrorSubscriptOfNonArray(Range location, EvaluatedType actualArrayType) => new(location, MessageCode.SubscriptOfNonArray,
    ...
    internal static Message ErrorUndefinedSymbol<TSymbol>(Ident identifier) where TSymbol : Symbol => new(identifier.Location, MessageCode.UndefinedSymbol,
        Fmt($"undefined {Symbol.GetKind<TSymbol>()} `{identifier}`"));

    internal static Message ErrorUndefinedSymbol<TSymbol>(Ident identifier, Symbol existingSymbol) where TSymbol : Symbol => new(identifier.Location,
        MessageCode.UndefinedSymbol,
        Fmt($"`{identifier}` is a {existingSymbol.Kind}, {Symbol.GetKind<TSymbol>()} expected"));
    internal static Message ErrorUnknownToken(Range location) => new(location, MessageCode.UnknownToken, new(input =>
    ...
    internal static Message ErrorInvalidCast(Range location, EvaluatedType sourceType, EvaluatedType targetType) => new(location, MessageCode.InvalidCast,
    ...
    internal static Message HintRedundantCast(Range location, EvaluatedType sourceType, EvaluatedType targetType) => new(location, MessageCode.RedundantCast,
    ...
    internal static Message ErrorExcessElementInInitializer(Range location) => new(location, MessageCode.ExcessElementInInitializer,
    ...
    internal static Message WarningDivisionByZero(Range location) => new(location, MessageCode.DivisionByZero,
        "division by zero will cause runtime error");

    internal static Message WarningFloatingPointEquality(Range location) => new(location, MessageCode.FloatingPointEquality,
        "floating point equality may be inaccurate",
        ["consider comparing absolute difference to an epsilon value instead"]);

    internal static Message HintUnofficialFeature(Range location, string feature, string? alternativeSolution = null) => new(location,
    ...
    internal static Message HintUnofficialFeatureScalarInitializers(Range location) => HintUnofficialFeature(location, "scalar initializers",
