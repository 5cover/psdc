...
public sealed partial class StaticAnalyzer
{
    ...
    ValueOption<InitializerPath.Structure, Message> EvaluateStructurePath(
        Scope scope,
        Designator.Structure first,
        ReadOnlySpan<Designator> rest,
    ...
    interface InitializerPath
    {
        ...
        Option<InitializerPath, Message> Advance(FixedRange location);

        Option<Value, Message> SetValue(FixedRange location, object haystack, Value needle);
        ...
        where TUnderlying : notnull
        {
            ...
            public Option<TSelf, Message> Advance(FixedRange location)
            {
            ...
            public Option<TValue, Message> SetValue(FixedRange location, TUnderlying haystack, Value needle) => SetValueFirst(location, haystack, Rest.Match(
            ...
            protected abstract ValueOption<TDesignator, Message> AdvanceFirst(FixedRange location);
            protected abstract Option<TUnderlying, Message> SetValueFirst(FixedRange location, TUnderlying haystack, ValueTransform transform);

            protected abstract TSelf WithRest(ValueOption<InitializerPath> rest);
            protected abstract TSelf WithFirst(TDesignator first);

            Option<InitializerPath, Message> InitializerPath.Advance(FixedRange location) => Advance(location);
            Option<Value, Message> InitializerPath.SetValue(FixedRange location, object haystack, Value needle) =>
        ...
        {
            public static ValueOption<Array, Message> OfFirstObject(ArrayType type, FixedRange location) => 0.Indexes(type.Length.Value)
                ? new Array(new(0), default, type)
                : Message.ErrorExcessElementInInitializer(location);
            protected override ValueOption<DesignatorInfo.Array, Message> AdvanceFirst(FixedRange location) => (First.Index + 1).Indexes(Type.Length.Value)
                ? new DesignatorInfo.Array(First.Index + 1)
                : Message.ErrorIndexOutOfBounds(location, First.Index + 2, Type.Length.Value);
            protected override Option<ImmutableArray<Value>, Message> SetValueFirst(FixedRange location, ImmutableArray<Value> haystack, ValueTransform transform) =>
        ...
        {
            public static ValueOption<Structure, Message> OfFirstObject(StructureType type, FixedRange location) => type.Components.List.FirstOrNone()
               .Map(comp => new Structure(new(comp.Key), default, type))
               .OrWithError(Message.ErrorExcessElementInInitializer(location));

            protected override ValueOption<DesignatorInfo.Structure, Message> AdvanceFirst(FixedRange location) => Type.Components.List.TryGetAt(1
            ...
            protected override Option<ImmutableOrderedMap<Ident, Value>, Message> SetValueFirst(
                FixedRange location,
                ImmutableOrderedMap<Ident, Value> haystack,
