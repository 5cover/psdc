...
switch in AnalyzeExpression : check for divizion by zero and other errors there.

but we already checked for those errors while evaluating the type? Should we ignore those error and detect them again ?

No we should not. So we will not call a EvaluateType extension method anymore. We will simply figure out the type in analyze expression and report all errors. We won't be constrained to a specific option type.

SO Remove EvaluateTypeOperationBinary and do proper logic in ConstantValue.

But what about the errors that occur when we try to evaluate the value of an expression ? Such as division by zero.

The core issue is that we want to report errors in AnalyzeExpression, but we don't care about the value. And we want the value in EvaluatedType, analysis of Declaration.Constant, and others.. but we don't care about the error.

So maybe what we should do is evaluate both the type and expression in AnalyzeExpression and return them. The return value will be discarded for most calls but not all of them.

AnalyzeExpression will return `Option<OneOf<EvaluatedType, ConstantValue>>` considering that a ConstantValue knows its type.

But implementing OneOf will be a pain. We can use `Option<T, TError>` as a replacement for now.


But what about when we expect a value of a specific type? Will still need the value evaluation logic, plus an error if the value was not of the expected type. Should we have 2 overloads of AnalyzeExpression, one with types and one without ? We will end up duplicating the logic.

But we can't really separate type and value evaluation from each other can we? Since if we evaluate the value we implicitly evaluate the type. But don't need the expression to be constant to evaluate its type.

We shouldn't evaluate expressions outside of StaticAnalyzer, just pass the results.

We can do both at the same time. Evaluate the type and value simulatenously first. If there's no value continue with just the type.

### constant expression

Wrap expressions that are known to be constant in ConstantExpression in:

- EvaluatedType.Array
- EvaluatedType.LenghtedString
- Symbol.Constant
- Analysis of Array.Subscript expressions

We need a generic ConstantExpression.

what should be the type of the result if one or both constant values are missing?
only ConstantValue knows...
we need a way to define for each binary operator and value type : how to calculate it and the type it gives.
currently what we do is we operate between constant values but that doesn't allow for types.
i don't want to duplicate the logic.

```cs
Dictionary<UnaryOperator, (
   Func<ConstantValue, ConstantValue, ConstantValue> compute,
   Func<EvaluatedType, EvaluatedType, EvaluatedType> getType)>
```

Maybe i'm overthinking all this.

All i want to do its evaluate the constant value of an expression, or at least its type.

Remove Type from constant value and return

`Option<(EvaluatedType, Option<ConstantValue>)>`

For binary operations :

1. Evaluate left operand
2. Evaluate right operand
3. If both have constant values, perform operation. (this will return both the value and the type)
4. Otherwise, perform type operation (`int == int` &rarr; `bool`...)
